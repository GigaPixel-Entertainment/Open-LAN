        ...
        async for message in ws:
            ...
            if "encryption" in msgDecoded and msgDecoded["encryption"] == "AES":
                    ...
                    for usr in users:
                        if usr["USRNAME"] == decryptedBody["username"]:
                            if bcrypt.checkpw(usrPwd, usr["PWD"].encode("utf-8")):
                                ...
                                SHORT_REDIRECT_TOKENS[usr["USRNAME"]] = {"TOKEN":token,"EXPIRES": time.time() + config.REDIRECT_TOKEN_EXPIRES_SEC}
                ...
                if decryptedBody["type"] == "signup":
                    ...
                    if not allowed:
                        await wsSendEncrypted(ws, orjson.dumps({"type": "signupFailed", "reason": f"You have been ratelimited. Please try again in {config.ACC_CREATION_COOLDOWN_SEC} minutes."}))
                        continue
                    ...
                    users.append({"UID": len(users), "USRNAME": username, "PWD": bcrypt.hashpw(password, bcrypt.gensalt(config.NUM_ENCRYPT_ROUNDS)).decode("utf-8"), "Displayname": username, "Birthday": None, "BirthdayV": "PRIVATE", "AccCreated": time.time(), "Pronouns": "", "Bio": "", "PFP": DEFAULT_PFPS[secrets.randbelow(len(DEFAULT_PFPS))], "Friends": [], "Chats": [0], "FriendRequests": []})

                    RATELIMITED_IPS.append({"ip": ws.remote_address[0], "expire": time.time() + config.ACC_CREATION_COOLDOWN_SEC})
...
async def shutdownWs(shutdownEvent: asyncio.Event, shutdownEventDone: asyncio.Event):
    shutdownEvent.set()
...
async def wsListen(ipAddrs: list, context: ssl.SSLContext, shutdownEvent: asyncio.Event, shutdownEventDone: asyncio.Event):
    ...
    for addr in ipAddrs:
        servers.append(serve(wsHandler, addr, config.WSS_PORT, max_size=(25*1024*1024 * 11), ssl=context, process_request=getAuth))
        logging.debug("[WS] wss://%s:%i", addr, config.WSS_PORT)
        servers.append(serve(wsHandler, addr, config.WS_PORT, max_size=(25*1024*1024 * 11), process_request=getAuth))
...
if __name__ == "__main__":
    ...
    DUMMY_HASH = bcrypt.hashpw(b"DUMMY_PW", bcrypt.gensalt(config.NUM_ENCRYPT_ROUNDS))
    ...
    asyncio.run_coroutine_threadsafe(shutdownWs(wsShutdownEvent, wsShutdownEventDone), loop=wsLoop)
    wsThread.join(10)
